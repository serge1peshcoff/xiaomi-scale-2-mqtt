...
class Measurement {
    constructor(params) {
        ...
        this.isStabilized = params.isStabilized;
        this.loadRemoved = params.loadRemoved;
        this.unit = params.unit;
        this.weight = params.weight;
        this.impedance = params.impedance;
    ...
    static createWithParams(dataFromScale, userInfo) {
        const age = userInfo.dateOfBirth
            ? moment().diff(userInfo.dateOfBirth, 'years', true)
            : undefined;

        return new Measurement({
            date: dataFromScale.date,
            isStabilized: dataFromScale.isStabilized,
            loadRemoved: dataFromScale.loadRemoved,
            unit: dataFromScale.unit,
            weight: dataFromScale.weight,
            impedance: dataFromScale.impedance,
            age,
    ...
    result() {
        ...
        params.bmi = this.getBMI();
        params.idealWeight = this.getIdealWeight();
        params.metabolicAge = this.getMetabolicAge();
        params.proteinPercentage = this.getProteinPercentage();
        params.lbm = this.getLBMCoefficient();
        params.bmr = this.getBMR();
        params.fatPercentage = this.getFatPercentage();
        params.muscleMass = this.getMuscleMass();
        params.boneMass = this.getBoneMass();
        params.visceralFat = this.getVisceralFat();
    ...
    getFatPercentage = () => {
        let value = 0.8
        if (this.sex === 'female' && this.age <= 49) {
            value = 9.25
        } else if (this.sex === 'female' && this.age > 49) {
            value = 7.25
        }
    ...
    getMuscleMass = () => {
        ...
        if (this.sex == 'female' && muscleMass >= 84) {
            muscleMass = 120;
        } else if (this.sex == 'male' && muscleMass >= 93.5) {
            muscleMass = 120;
        }
